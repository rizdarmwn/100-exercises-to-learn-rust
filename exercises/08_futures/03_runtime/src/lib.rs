    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle_first = tokio::spawn(freply(first, reply.clone()));
    let handle_second = tokio::spawn(freply(second, reply.clone()));

    let _ = tokio::join!(handle_first, handle_second);
}

async fn freply<T>(listener: TcpListener, reply: T)
where
    T: Display + Send + Sync + 'static,
{
    loop {
